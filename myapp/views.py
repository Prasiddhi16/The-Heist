...
def _get_or_create_submission(
    ...
    SubmissionSuspect.objects.bulk_create([
        ...
        for suspect in Suspect.objects.filter(
            case_id=case_id
        )
    ])
...
def toggle_evidence(
    ...
    row, created = (
        SubmissionEvidence.objects.get_or_create(
            submission=submission,
            evidence=evidence
        )
    )
...
def solve_case(
    ...
    if (
        ...
        if (
            suspect_correct
            and not evidence_correct
        ):
            submission.reviewer_notes = (
                "Correct suspect, but the "
            ...
            submission.reviewer_notes = (
                "Correct evidence selected, but "
                "the accused suspect does not "
                "match the official solution."
            )

        else:
            submission.reviewer_notes = (
                "The selected suspect and evidence "
    ...
    if user_id:
        ...
        if user_profile:
            ...
            total_score = (
                completed_submissions.aggregate(
                    total=Sum("score")
                )["total"]
                or 0
            )
